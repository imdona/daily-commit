...
N, M = map(int, sys.stdin.readline().split())  # N : 나무의 수 / M : 필요한 나무의 길이
trees = Counter(list(map(int, sys.stdin.readline().split())))
# print(trees)
# print(trees.items())  # dict_items([(20, 1), (15, 1), (10, 1), (17, 1)])
...
def binary_search_tree(start, end):
    ...
    mid = (start + end) // 2
    total = sum((i - mid)*j for i, j in trees.items() if i > mid)

    if total >= M:  # 자른 나무 >= 필요나무
        return binary_search_tree(mid + 1, end)
    elif total < M:  # 자른나무 < 필요나무
        return binary_search_tree(start, mid - 1)

start, end = 0, max(trees)  # 시작, 끝
print(binary_search_tree(start, end))
