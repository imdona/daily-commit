'''
백준 7568 [덩치] - silver 5 & 구현, 브루트포스 알고리즘
...
# 순위 매기기 : 순위를 2번째 인덱스로 추가해서 넣어준다. 조건은 간단하다. 나보다 크면 count를 +1 해준다.
for i in people:
    count = 0
    for j in people:
        if i[0] < j[0] and i[1] < j[1]:
...
# 순위 출력 : 2번째 인덱스의 순위만 뽑아 하나의 리스트를 만들고, unpacking하여 출력한다.
rank = []
for i in people:
    rank.append(i[2])
...
################################################################
'''마지막에 프린트 문에서 조금 더 간결하게 작성할 수 있다
for 문에서 바로 프린트를 해주는데, end = " "를 추가해서 끝에 스페이스(화이트 스페이스)를 추가해준다.'''
import sys
N = int(sys.stdin.readline())
people = []
for _ in range(N):
